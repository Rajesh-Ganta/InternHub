...
export class ApplicationPreviewComponent implements OnInit {
  ...
  apply()
  {
    ...
    {
      this.http.post("http://localhost:8000/insert_application",{"student_id": sid, "name": 'Rajesh', "email" : email, "phone": "123434", "company_name": this.data["company_name"], "notice_id": this.data["notice_id"]}).subscribe((res)=>{
        console.log(res);
      },(err)=>{
        console.log(err);
      },()=>{
