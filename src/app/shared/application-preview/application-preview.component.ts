...
export class ApplicationPreviewComponent implements OnInit {
  ...
  apply()
  {
    ...
    {
      this.http.post("http://192.168.224.100:8000/insert_application",{"student_id": sid, "name": sid, "email" : email, "phone": phone, "company_name": this.data["company_name"], "notice_id": this.data["notice_id"]}).subscribe((res)=>{
        console.log(res);
      },(err)=>{
        console.log(err);
      },()=>{
